...
class GoogleBooksApi {
  // static final getBookByNameProvider =
  //     FutureProvider.autoDispose.family<List<Book>, String>((ref, name) async {
  //   return getBooksByName(name);
  // });

  static Future<void> retrieveAllGoogleBooks() async {
    final maxResults = 40;
    var startIndex = 0;
    ...
    while (true) {
      ...
      final books =
          data['items'] == null ? null : data['items'] as List<dynamic>;
      log(books.toString());

      if (books == null) break;
      List<Book> booksItems = [];
      final apiBooks = List<BookApi>.from(
          (data["items"] as List<dynamic>).map<BookApi?>((x) {
  ...
  static Future<List<Book>> getBooksByName(
      String name, int startIndex, int maxResults) async {
    final response = await http.get(
      Uri.parse(
          'https://www.googleapis.com/books/v1/volumes?q=$name&startIndex=$startIndex&maxResults=$maxResults'),
    );
    ...
    final apiBooks =
        List<Book>.from((data["items"] as List<dynamic>).map<Book?>((x) {
      final bookApi = BookApi.fromJson(x);
      final repoBook = Book(
        id: bookApi.id,
        name: bookApi.volumeInfo?.title,
